...
namespace Lof\AffiliateSaveCart\Block;
...
class Cart extends \Magento\Checkout\Block\Cart\AbstractCart
{
    ...
    {
        if ($extensionAttributes = $this->getQuote()->getExtensionAttributes()) {
            return $extensionAttributes->getSaveCartData();
        }
    ...
    public function getAccountAffiliate()
    {
        $customer_id = $this->getQuoteSaveCartData()->getCustomerId();
        $account_model = $this->_accountAffiliate->create()->loadByAttribute('customer_id', $customer_id);
        return $account_model;
    }
    public function isEnableShareSaveCart() {
        $enable = $this->saveCartHelper->getConfig('affiliatesavecart/enable');
        return $enable;
    }
    public function getShareSaveCartRoute() {
        $route = $this->saveCartHelper->getConfig('affiliatesavecart/sharecart_route');
        if($route) {
            $route = trim($route);
