...
class Programs extends \Magento\Framework\View\Element\Template
{
    /**
     * @var Session
     */
    ...
    public function getTotalCommission($campaignCode)
    {
        ...
        $affiliateAccount = $this->accountAffiliate->create()->getCollection()
            ->getItemByColumnValue('email', $customerEmail);

        if ($affiliateAccount) {
            $referCode = $affiliateAccount->getTrackingCode();
        }

        if (!$referCode) {
            // Return 0 or handle the case where the refer code is not found
            return 0;
        }
