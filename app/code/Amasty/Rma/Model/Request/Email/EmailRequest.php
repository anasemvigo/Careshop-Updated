...
class EmailRequest extends DataObject
{
    ...
    private function prepareCustomFields($request)
    {
        ...
        foreach ($customFields as $code => $value) {
            if (isset($configCustomFields[$code])) {
                $result[] = [
                    'label' => $configCustomFields[$code],
                    'value' => $value
                ];
            }
        }
    ...
    {
        $url = $this->storeManager->getStore($request->getStoreId())->getBaseUrl();

        if ($customerId = $request->getCustomerId()) {
            $url .= $this->configProvider->getUrlPrefix() . '/account/view/request/' . $request->getRequestId();
        } else {
            $url .= $this->configProvider->getUrlPrefix() . '/guest/view/request/' . $request->getUrlHash();
        }
