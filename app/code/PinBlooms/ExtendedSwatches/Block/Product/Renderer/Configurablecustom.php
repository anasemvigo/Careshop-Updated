...
namespace PinBlooms\ExtendedSwatches\Block\Product\Renderer;
 ...
 * @since 100.0.2
 */
class Configurablecustom extends \Magento\ConfigurableProduct\Block\Product\View\Type\Configurable implements
    \Magento\Framework\DataObject\IdentityInterface
{
    /**
     * Path to template file with Swatch renderer.
     */
    const SWATCH_RENDERER_TEMPLATE = 'Magento_Swatches::product/view/renderer_custom.phtml';
     ...
     * @return string
     */
    public function getDataSwatchColor($color_id = false)
    {
        $attributesData = $this->getSwatchAttributesData();
        $data = [];

        if ($color_id) {
            if (isset($attributesData[$color_id]['options']) && count($attributesData[$color_id]['options']) > 0) {
                $in_ar = [];
                foreach ($attributesData[$color_id]['options'] as $key => $val) {
                    $in_ar[] = $key;
                }
                $data = $this->swatchCollection->addFieldtoFilter('option_id', ['in'=> $in_ar ]);
            }
        }
