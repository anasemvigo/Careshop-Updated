...
class TransactionService extends AbstractTransactionService
{
    ...
    private function convertQuoteBillingAddress(Quote $quote)
    {
        ...
        $logger->info('start11');
        $logger->info($this->getGender($quote->getCustomerGender(), $quote->getCustomerId()));
    ...
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/convertAddress.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $quote = $this->checkoutSession->getQuote();
        $gender = $quote->getData('customer_gender');
        $numeric = is_numeric($gender);
        $logger->info('Gender:'.$gender);
        $logger->info('Numeric:'.$numeric);
        if($numeric) {
            $gender = $this->getGenderFromNum($gender);
        }
        $logger->info('New Gender:'.$gender);
    ...
    public function getGenderFromNum($gender)
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/convertAddressfn.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info('New Gender:'.$gender);
